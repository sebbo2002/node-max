module.exports = function(shared) {
	...
	var BitsHelper = require('../helpers/bits.js');
	...
	return function CubeHandleConfiguration(a) {
		...
		if([1, 2].indexOf(deviceData.type) > -1) {
			...
			deviceData.boostDuration = parseInt(new BitsHelper(deviceConfig, pointer).splice(0, 2), 2) * 5;
			deviceData.boostValve = parseInt(new BitsHelper(deviceConfig, pointer).splice(3, 7), 2) * 5;
			...
			// Saturday = 0
			deviceData.decalcificationDay = parseInt(new BitsHelper(deviceConfig, pointer).splice(0, 2), 2);
			deviceData.decalcificationHour = parseInt(new BitsHelper(deviceConfig, pointer).splice(3, 7), 2);
			pointer += 1;
